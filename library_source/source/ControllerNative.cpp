...
JNIEXPORT void JNICALL Java_controller_ControllerNative_moveMirror
  (JNIEnv * env, jobject java_this, jint row, jint col, jint motor, jint direction, jint step){
  ...
  if(jBlockMode == NULL){
    std::cout << "Failed to find the field named \"blockMode\" in the java class \"ControllerNative\"\n";
  ...
  try{
      if(jBlockValue == JNI_TRUE){
         ...
         std::cout << "Push Command to the queue" << std::endl;
         PendingCommands.push([row, col, motor, direction, step](CatoptricSurface* that){
            std::cout << "row: " << row << " col: " << col << " motor: " << motor << " diretion: " << direction << " step: " << step <<std::endl;
      ...
      if(env != NULL)env->ThrowNew(java_exec, e.what());
      else{
...
JNIEXPORT void JNICALL Java_controller_ControllerNative_reset
  (JNIEnv *, jobject){
    std::cout <<"native method reset invoked"<<std::endl;
...
JNIEXPORT void JNICALL Java_controller_ControllerNative_test
  (JNIEnv *, jobject){
    std::cout <<"native method test invoked" << std::endl;
